...
```{r}
seep.dating = read_csv("./dados/speed-dating.csv") 
seep.dating %>% 
  filter(like != 'NA', intel != 'NA', sinc != 'NA') %>% 
  select(like, intel, sinc) %>% 
...
```{r}
intel.model = lm(like ~ intel + fun, data = seep.dating)

adv_augmented = seep.dating %>% 
